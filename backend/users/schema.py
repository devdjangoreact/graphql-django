...
from config.authentication import TokenManager
from config.permissions import is_authenticated, paginate
...
class CreateUserProfile(graphene.Mutation):
    ...
    def mutate(self, info, profile_data, **kwargs):
        user_profile = UserProfile.objects.create(user_id=info.context.user.id, **profile_data, **kwargs)
...
class CreateUserAddress(graphene.Mutation):
    ...
    def mutate(self, info, address_data, is_default=False):
        ...
        address = UserAddress.objects.create(user_profile_id=user_profile_id, is_default=is_default, **address_data)
