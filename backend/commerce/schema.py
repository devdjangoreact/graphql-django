...
from config.permissions import paginate, is_authenticated, get_query
from django.db.models import Q

from .models import Category, Business, Product, ProductComment, ProductImage, Wish, Cart, RequestCart
...
class Query(graphene.ObjectType):
    ...
    def resolve_request_carts(self, info, name=False):
        query = RequestCart.objects.select_related("user", "product", "business").filter(
            business__user_id=info.context.user.id
        )
    ...
    def resolve_product(self, info, id):
        query = (
            Product.objects.select_related("category", "business")
            .prefetch_related(
                "product_images", "product_comments", "products_wished", "product_carts", "product_requests"
            )
            .get(id=id)
...
class CreateBusiness(graphene.Mutation):
    ...
    def mutate(self, info, name):
        ...
        return CreateBusiness(business=buss)
...
class UpdateBusiness(graphene.Mutation):
    ...
    def mutate(self, info, name):
        ...
        return UpdateBusiness(business=instance)
...
class CreateProduct(graphene.Mutation):
    ...
    def mutate(self, info, total_count, product_data, images, **kwargs):
        ...
        ProductImage.objects.bulk_create([ProductImage(product_id=product.id, **image_data) for image_data in images])
...
class UpdateProductImage(graphene.Mutation):
    ...
    def mutate(self, info, image_data, id):
        ...
        return UpdateProductImage(image=ProductImage.objects.get(id=id))
...
class CreateProductComment(graphene.Mutation):
    ...
    def mutate(self, info, product_id, **kwargs):
        ...
        return CreateProduct(product_comment=pc)
...
class UpdateCartItem(graphene.Mutation):
    ...
    def mutate(self, info, cart_id, **kwargs):
        ...
        return UpdateCartItem(cart_item=Cart.objects.get(id=cart_id))
...
class DeleteCartItem(graphene.Mutation):
    ...
    def mutate(self, info, cart_id):
        ...
        return DeleteCartItem(status=True)
